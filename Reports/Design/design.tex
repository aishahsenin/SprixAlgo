...
As when the animation of the knapsack problem starts (refer to figure \ref{animationDesignKnapsackProblem} and figure \ref{animationDesignKnapsackProblem_STEP1}), there will be a few texts which describes the tables that are found within the animation. Those texts found around the table basically describes the use of them, in order for the user to understand what they are for before the actual algorithm starts. These texts will remain here for about a 30 seconds, before proceeding to the next state.
...
Included in this animation, other than the ones mentioned before, are the items represented in cubes. Along with them are its values, which are its respective values and weight. Next to it is an image of a rucksack, that basically represents the knapsack, with values such as its maximum capacity, and the current weight it is holding. For its initial state, the current value is always 0.
