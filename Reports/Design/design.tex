...
Another feature that is available in the program would be the settings feature. Based on the sequence diagram in figure \ref{sequenceDiagramSettings}, when the user opens the program, the program will call the \textit{displayMenu()} method to display the main menu onto the client. For this case, the user then selects \textit{Settings} button, which leads them to the settings page, called out by the \textit{displaySettings()} method.

In the settings page, the user can change features such as the font size and the animation speed. When the user decides to change the font size by using the drag controller, the \textit{changeFontSize()} method is being called out, with the current value depending on where the selector along the slider. Once that is done, the program then calls out the \textit{displayUpdatedSettings()} to change the outlook of the program to whatever that is changed. The same applies to when the speed of animation is changed, however this time, the \textit{changeAnimationSpeed()} method is called out instead. 

Once the user is done with changing the settings, they could press the \textit{Back} button, which leads them back to the main menu. To see the graphical interface design of the settings page, do refer to figure \ref{uiSettings}, on page \pageref{uiSettings}.
...
Last but not least, the appendix would be the other feature that would be available in the \textit{Algorithm Animation} program. As per usual, as when the user initiates the program, the user would be led to the main menu. Once there, the user then clicks the \textit{View appendix} button, which calls out the \textit{displayAppendixList()} method, and leads the user to the appendix page.

Once the appendix page has been displayed, the user then can select the algorithm they wish to find out more about, within the list presented on the page. Once the user clicks one, the program calls out the \textit{displayAlgorithmInfo()} with the name of the algorithm as the passing parameter, which displays the information available for the algorithm.

When the user is done, the \textit{Back} button can be selected, which leads them back to the main menu. If you wish to refer to the graphical user interface design of the appendix page, refer to figure \ref{uiAppendix} in page \pageref{uiAppendix}.
...
As when the calculation is complete, and the knapsack's current weight has reached to its max weight, the animation then comes into a conclusion. The conclusion of this algorithm is that the knapsack is finally full, and the total value that the knapsack has achieved is displayed as well. Afterwards, the animation stops here. This conclusion is shown in the in figure \ref{animationDesignFractionalKnapsackProblem_STEP5}, on page \pageref{animationDesignFractionalKnapsackProblem_STEP5}.
...
\centering
%\hspace*{-0.5cm}
\includegraphics[scale=0.85]{images/report_images/animationDesignKnapsackProblem.png}
...
\centering
%\hspace*{-0.5cm}
\includegraphics[scale=1]{images/report_images/animationDesignKnapsackProblem_STEP1.png}
\caption{The animation design of the Knapsack Problem.}
